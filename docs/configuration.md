...
The old `inputs.ancestry_mode`, `inputs.ancestry_file`, `inputs.pcs_file`,
`inputs.projected_pcs_file`, and `inputs.reference_pcs_file` fields are no
longer supported. Ancestry assignment and PC covariates are always generated by
the package-backed workflow.
...
| `min_markers` | Recommended | Minimum informative markers required before accepting a build call. Template uses `50`; internal unit tests may override this threshold. |
