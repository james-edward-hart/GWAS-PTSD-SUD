...
from workflow.snake_helpers import (
    ...
    remeta_group_ld_inputs as remeta_group_ld_inputs_for_config,
...
PHASE2_GROUPS = phase2_group_ids_for_config(config)
...
phase2_status_file = lambda wildcards: phase2_status_file_for_context(checkpoints)
phase2_trait_group = lambda wildcards: phase2_trait_group_for_config(config, wildcards.trait)
phase2_group_trait = lambda wildcards: phase2_group_traits_for_config(config, wildcards.group)[0]
remeta_targets = lambda wildcards: remeta_targets_for_context(checkpoints, wildcards, config)
remeta_manifest_inputs = lambda wildcards: remeta_manifest_inputs_for_config(checkpoints, config, wildcards.build)
remeta_group_htp_inputs = lambda wildcards: remeta_group_htp_inputs_for_config(config, wildcards.group, wildcards.build)
remeta_group_ld_inputs = lambda wildcards: remeta_group_ld_inputs_for_config(wildcards.group, wildcards.build)
...
optional_path_args = lambda flag, paths: " ".join(
    f"{flag} {shlex.quote(str(path))}" for path in paths
)
